...
(function (Kubernetes) {
    ...
    Kubernetes.defaultIconUrl = Core.url("/" + Kubernetes.pluginPath + "img/kubernetes.svg");
     ...
     */
    function createPodCounters(selector, pods, outputPods) {
...
(function (Kubernetes) {
        ...
        KubernetesModelService.prototype.maybeInit = function () {
            ...
            if (this.services && this.replicationControllers && this.pods) {
                ...
                this.services.forEach(function (service) {
                    ...
                    var selectedPods = selectPods(_this.pods, service.namespace, service.selector);
     ...
     */
    function createKubernetesModel($rootScope, $http, AppLibraryURL, KubernetesState, KubernetesServices, KubernetesReplicationControllers, KubernetesPods) {
...
(function (Kubernetes) {
    ...
    Kubernetes._module.factory('KubernetesModel', ['$rootScope', '$http', 'AppLibraryURL', 'KubernetesState', 'KubernetesServices', 'KubernetesReplicationControllers', 'KubernetesPods', function ($rootScope, $http, AppLibraryURL, KubernetesState, KubernetesServices, KubernetesReplicationControllers, KubernetesPods) {
        return Kubernetes.createKubernetesModel($rootScope, $http, AppLibraryURL, KubernetesState, KubernetesServices, KubernetesReplicationControllers, KubernetesPods);
    }]);
    Kubernetes._module.run(['viewRegistry', 'workspace', 'ServiceRegistry', 'HawtioNav', function (viewRegistry, workspace, ServiceRegistry, HawtioNav) {
        ...
        var mainTab = builder.id('kubernetes').href(function () { return Kubernetes.context; }).title(function () { return 'Kubernetes'; }).isValid(function () { return Kubernetes.isKubernetes(workspace); }).tabs(apps, services, controllers, pods, overview).build();
    ...
    Kubernetes.Apps = Kubernetes.controller("Apps", ["$scope", "KubernetesModel", "KubernetesServices", "KubernetesReplicationControllers", "KubernetesPods", "KubernetesState", "KubernetesApiURL", "$templateCache", "$location", "$routeParams", "$http", "$dialog", "$timeout", "workspace", "jolokia", function ($scope, KubernetesModel, KubernetesServices, KubernetesReplicationControllers, KubernetesPods, KubernetesState, KubernetesApiURL, $templateCache, $location, $routeParams, $http, $dialog, $timeout, workspace, jolokia) {
        ...
        $scope.resizeDialog = {
            ...
            onOk: function () {
                ...
                Kubernetes.resizeController($http, KubernetesApiURL, $scope.resizeDialog.resize.controller.id, $scope.resizeDialog.resize.newReplicas, function () {
...
(function (Kubernetes) {
    ...
    var ReplicationControllerIcon = Kubernetes.controller("ReplicationControllerIcon", ["$scope", "jolokia", function ($scope, jolokia) {
        $scope.iconUrl = Kubernetes.defaultIconUrl;
        jolokia.request({
            type: 'exec',

...
(function (Kubernetes) {
    ...
    function kubernetesUrlForKind(KubernetesApiURL, kind, namespace, path) {
        ...
        if (isV1beta1Or2()) {
            ...
            return UrlHelpers.join(KubernetesApiURL, "/api/" + Kubernetes.defaultApiVersion + "/" + lowerKind + pathSegment + postfix);
        }
        else {
            return UrlHelpers.join(KubernetesApiURL, "/api/" + Kubernetes.defaultApiVersion + "/ns/" + namespace + "/" + lowerKind + pathSegment + postfix);
        }
    }
...
var Kubernetes;
(function (Kubernetes) {
    Kubernetes.Services = Kubernetes.controller("Service", ["$scope", "KubernetesModel", "KubernetesState", "$templateCache", "$location", "$routeParams", "$http", "$timeout", "KubernetesApiURL", function ($scope, KubernetesModel, KubernetesState, $templateCache, $location, $routeParams, $http, $timeout, KubernetesApiURL) {
        $scope.kubernetes = KubernetesState;
        $scope.model = KubernetesModel;
        Kubernetes.initShared($scope, $location, $http, $timeout, $routeParams, KubernetesModel, KubernetesState, KubernetesApiURL);
        $scope.$on('kubernetesModelUpdated', function () {

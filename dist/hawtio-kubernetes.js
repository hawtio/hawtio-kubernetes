...
(function (Kubernetes) {
    ...
    Kubernetes.deploymentConfigsRestURL = "/kubernetes/osapi/" + Kubernetes.defaultOSApiVersion + "/deploymentConfigs";
    ...
    function enrichBuildConfig(KubernetesApiURL, buildConfig) {
        if (buildConfig) {
            var triggerUrl = null;
            var name = Core.pathGet(buildConfig, ["metadata", "name"]);
            if (name) {
                            ...
                            if (!triggerUrl) {
                                ...
                                if (type === "generic" || flag) {
                                    ...
                                    if (type && generic) {
                                        ...
                                        if (secret) {
                                            triggerUrl = UrlHelpers.join(KubernetesApiURL, 'osapi', Kubernetes.defaultOSApiVersion, 'buildConfigHooks', name, secret, type);
        ...
        function updateData() {
            ...
            if ($scope.id) {
                ...
                $http.get(url).success(function (data, status, headers, config) {
                    if (data) {
                        $scope.entity = data;
                        Kubernetes.enrichBuildConfig(KubernetesApiURL, data);
                    }
                    $scope.fetched = true;
                    Core.$apply($scope);
                }).error(function (data, status, headers, config) {
            ...
            $http.get(url).success(function (data, status, headers, config) {
                if (data) {
                    ...
                    angular.forEach($scope.buildConfigs, function (buildConfig) {
                        Kubernetes.enrichBuildConfig(KubernetesApiURL, buildConfig);
                    });
                    $scope.fetched = true;
                }
            }).error(function (data, status, headers, config) {
